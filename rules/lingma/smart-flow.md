...
> 生成时间：2026-04-06T16:19:27.840Z

## 功能说明

本规则提供 36 个标准化工作流节点（Skill），AI 助手根据用户输入的触发词自动执行对应流程。
...
- **触发词**：代码审查、code review、CR、帮我 review、帮我看看代码
...
- **触发词**：接口设计、API 设计、api design、设计接口、接口规范
...
- **触发词**：安全审查、安全扫描、安全检查、漏洞扫描、security review
...
- **触发词**：PRD、产品需求、写 PRD、需求文档、prd 编写
...
- **触发词**：Git 工作流、git workflow、git 规范、分支策略、branching strategy
...
- **触发词**：单元测试、unit test、写测试、write tests、TDD
...
- **触发词**：系统设计、system design、架构设计、architecture design、高并发系统
...
- **触发词**：数据库优化、database optimize、慢查询、slow query、SQL 优化
...
- **触发词**：Docker、docker、容器化、containerization、Dockerfile
...
- **触发词**：CI/CD、cicd、流水线、pipeline、持续集成
...
- **触发词**：性能优化、performance、页面慢、接口慢、性能分析
...
- **触发词**：代码重构、refactoring、refactor、重构、坏味道
...
- **触发词**：可观测性、observability、监控、monitoring、日志
...
- **触发词**：设计模式、design pattern、design patterns、模式、GoF
...
- **触发词**：spec proposal、openspec proposal、生成提案、变更提案、spec 提案
...
- **触发词**：spec review、intent review、意图审查、openspec review、spec 审查
...
- **触发词**：技术债、tech debt、technical debt、技术债追踪、技术债偿还
...
- **触发词**：api mock、mock service、mock 服务、接口 mock、msw
...
- **触发词**：数据迁移、data migration、schema migration、db migration、数据库迁移
...
- **触发词**：llm feature、ai feature、rag、llm 功能、ai 功能设计
...
- **触发词**：威胁建模、threat model、threat modeling、stride、攻击面分析
...
- **触发词**：绿色编码、green code、green software、carbon footprint、能耗优化
...
- **触发词**：服务目录、service catalog、service registry、backstage、内部开发者平台
...
- **触发词**：移动端审查、mobile review、ios review、android review、flutter review
...
- **触发词**：数据管道、data pipeline、etl、elt、data engineering
...
- **触发词**：ml experiment、mlops、model training、模型训练、实验追踪
