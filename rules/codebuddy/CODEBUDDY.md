...
**触发词**：接口设计 / API 设计 / api design / 设计接口

**目标**：基于业务需求设计清晰、可演进的 RESTful / GraphQL 接口规范，输出接口文档
...
**触发词**：安全审查 / 安全扫描 / 安全检查 / 漏洞扫描

**目标**：基于 OWASP Top 10 对代码和依赖进行安全扫描，识别漏洞并给出修复建议
...
**触发词**：部署上线 / 上线 / 发布 / deploy

**目标**：系统化执行部署上线流程，覆盖预检、发布、验证和回滚，保障变更安全落地
...
**触发词**：PRD / 产品需求 / 写 PRD / 需求文档

**目标**：从用户故事和业务目标出发，结构化生成产品需求文档（PRD），支撑研发高效落地
...
**触发词**：Git 工作流 / git workflow / git 规范 / 分支策略

**目标**：规范 Git 分支策略、提交规范、合并流程，建立团队一致的版本控制工作流
...
**触发词**：单元测试 / unit test / 写测试 / write tests

**目标**：运用 AAA 模式和 TDD 工作流编写高质量单元测试，建立覆盖率目标和 Mock 策略
...
**触发词**：系统设计 / system design / 架构设计 / architecture design

**目标**：从需求澄清到架构设计全流程，完成高并发分布式系统的方案设计与权衡分析
...
**触发词**：数据库优化 / database optimize / 慢查询 / slow query

**目标**：系统诊断数据库性能问题，涵盖 Schema 审查、索引设计、慢查询分析和 N+1 修复
...
**触发词**：Docker / docker / 容器化 / containerization

**目标**：编写生产级 Dockerfile，实现多阶段构建、镜像优化和 docker-compose 编排
...
**触发词**：CI/CD / cicd / 流水线 / pipeline

**目标**：设计完整 CI/CD 流水线，涵盖流水线阶段设计、测试自动化、部署门控和回滚策略
...
**触发词**：性能优化 / performance / 页面慢 / 接口慢

**目标**：系统化分析和优化前后端性能瓶颈，涵盖分析工具使用、优化策略和量化指标
...
**触发词**：代码重构 / refactoring / refactor / 重构

**目标**：系统化识别代码坏味道，运用重构手法安全改善代码结构，不改变外部行为
...
**触发词**：可观测性 / observability / 监控 / monitoring

**目标**：建立日志、指标、链路追踪三支柱体系，实现系统状态完全可观测，快速定位生产问题
...
**触发词**：设计模式 / design pattern / design patterns / 模式

**目标**：识别适用场景，选择合适的 GoF 设计模式，提升代码可扩展性与可维护性
